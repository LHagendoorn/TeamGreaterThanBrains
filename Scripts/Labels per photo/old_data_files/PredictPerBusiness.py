...
LabelsForPhotos.drop(LabelsForPhotos.columns[[0]], axis=1, inplace=True)
LabelsForPhotos['photo_id'] = LabelsForPhotos['photo_id'].map(lambda x: str(x)[:-5]) #remove m.jpg
LabelsForPhotos['photo_id'] = LabelsForPhotos['photo_id'].astype(int) #set type of photoids to int
blaa=LabelsForPhotos['photo_id'][5]
...
for x in range(0,NrUniqueBus):
    ...
    if maxnrlabels==0:
        ...
        normhist=floatsumlabels/len(photoids)
