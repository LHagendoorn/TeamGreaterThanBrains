...
Extract Histogram of Oriented Gradients (HOG) for all images in the testdata.
...
filename = 'HOG_features_test_6_10_1.csv'

#load file names of all test photos 
foto_ids= Input.load_testdata_filenames()
...
for x in range(nr_of_photos):
    ...
    image_color = Input.get_image_by_filename(current_photo, True)
    image = color.rgb2gray(image_color)

    #calculate the Histogram of Oriented Gradients (HOG) for current image
    fd = hog(image, orientations=6, pixels_per_cell=(10, 10),
