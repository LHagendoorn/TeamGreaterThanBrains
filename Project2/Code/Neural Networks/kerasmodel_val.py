# -*- coding: utf-8 -*-
# Run Keras model (vgg16) on trainset and validationset#
...
def get_driver_data():
    dr = dict()
    path = os.path.join('..', 'input', 'trainset_driver_imgs_list.csv')
...
def load_train(img_rows, img_cols, color_type=1):
    ...
    for j in range(10):
        ...
        path = os.path.join('..', 'input', 'imgs', 'train_valset',
                            'c' + str(j), '*.jpg')
...
def load_test(img_rows, img_cols, color_type=1):
    ...
    path = os.path.join('..', 'input', 'imgs', 'test_valset', '*.jpg')
    files = glob.glob(path)
    X_test = []
    X_test_id = []
    total = 0
